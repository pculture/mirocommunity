...
class BaseVideoListNode(template.Node):
    ...
    #: A sort to use on the queryset generated by this tag.
    sort = None
    #: If filter_name is provided, then the tag will take an additional
    #: argument: a value for that filter.
    filter_name = None
    ...
    def get_video_list(self, context):
        ...
        if self.sort is not None:
            form_data['sort'] = self.sort
        if self.filter_name is not None:
            form_data[self.filter_name] = [self.item.resolve(context)]
