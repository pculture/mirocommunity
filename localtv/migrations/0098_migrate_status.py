...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.Video.objects.filter(old_status=UNAPPROVED).update(status=NEEDS_MODERATION)
        orm.Video.objects.filter(old_status=ACTIVE).update(status=PUBLISHED)
        orm.Video.objects.filter(old_status=REJECTED).update(status=HIDDEN)
        orm.Video.objects.filter(old_status=PENDING).update(status=UNPUBLISHED)

    def backwards(self, orm):
        "Write your backwards methods here."
        orm.Video.objects.filter(status=NEEDS_MODERATION).update(old_status=UNAPPROVED)
        orm.Video.objects.filter(status=PUBLISHED).update(old_status=ACTIVE)
        orm.Video.objects.filter(status=HIDDEN).update(old_status=REJECTED)
        orm.Video.objects.filter(status=UNPUBLISHED).update(old_status=PENDING)
