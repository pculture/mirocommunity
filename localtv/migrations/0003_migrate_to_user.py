...
class Migration:
    ...
    def forwards(self, orm):
        ...
        for openid_user in orm.OpenIdUser.objects.all():
            try:
                user = orm['auth.User'].objects.get(username=openid_user.nickname)
        ...
        for saved_search in orm.SavedSearch.objects.filter(openid_user__isnull=False):
            saved_search.user = saved_search.openid_user.user
            saved_search.save()

        for feed in orm.Feed.objects.filter(openid_user__isnull=False):
            feed.user = feed.openid_user.user
            feed.save()

        for video in orm.Video.objects.filter(openid_user__isnull=False):
            video.user = video.openid_user.user
            video.save()

        for watch in orm.Watch.objects.filter(openid_user__isnull=False):
            watch.user = video.openid_user.user
            watch.save()

        for sitelocation in orm.SiteLocation.objects.all():
            sitelocation.admins_user = [oid.user for oid in
                                        sitelocation.admins.all()]
            sitelocation.save()

    def backwards(self, orm):
        "Write your backwards migration here"
        for saved_search in orm.SavedSearch.objects.filter(openid_user__isnull=False):
            saved_search.openid_user = saved_search.user.openiduser
            saved_search.save()

        for feed in orm.Feed.objects.filter(openid_user__isnull=False):
            feed.openid_user = feed.user.openiduser
            feed.save()

        for video in orm.Video.objects.filter(openid_user__isnull=False):
            video.openid_user = video.user.openiduser
            video.save()

        for watch in orm.Watch.objects.filter(openid_user__isnull=False):
            watch.openid_user = video.user.openiduser
            watch.save()

        for sitelocation in orm.SiteLocation.objects.all():
            sitelocation.admins = [u.openiduser for u in
                                        sitelocation.admins_user.all()]
            sitelocation.save()
    ...
    models = {
        ...
        'auth.user': {
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2009, 7, 6, 18, 57, 11, 651324)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2009, 7, 6, 18, 57, 11, 651189)'}),
