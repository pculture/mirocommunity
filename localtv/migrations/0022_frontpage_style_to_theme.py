...
class Migration:
    ...
    def forwards(self, orm):
        ...
        if sitelocation.frontpage_style == 'list':
            theme_id = 1
        elif sitelocation.frontpage_style == 'scrolling':
            theme_id = 2
        elif sitelocation.frontpage_style == 'categorized':
            theme_id = 3

        if theme_id is not None:
            default = orm['uploadtemplate.Theme'].objects.get(default=True)
            if default.pk == theme_id:
                default.site = sitelocation.site
                default.save()
            else:
                default.default = False
                default.save()
                t = orm['uploadtemplate.Theme'].objects.get(pk=theme_id)
                t.site = sitelocation.site
                t.default = True
                t.save()
    ...
    complete_apps = ['uploadtemplate', 'localtv']
