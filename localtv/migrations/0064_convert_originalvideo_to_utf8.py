...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """This method goes straight to the database, bypassing all ORM stuff,
        and just passes messages straight to the MySQL database."""
        cursor = connection.cursor()
        try:
            cursor.execute("ALTER TABLE localtv_originalvideo CONVERT TO CHARACTER SET utf8;")
        except Exception, e:
            logging.error(e)
            logging.error("For some reason, we failed to run this migration. It's probably not a big deal, though.")

    def backwards(self, orm):
        "In my opinion, once you go UTF-8, there is no going back."
        pass
