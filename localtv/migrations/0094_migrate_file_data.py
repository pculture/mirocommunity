...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        VideoFile = orm['localtv.videofile']
        for video in Video.objects.exclude(file_url=''):
            try:
                video_file = video.videofile_set.get(url=video.file_url)
            except VideoFile.DoesNotExist:
                video_file = VideoFile(url=video.file_url, video=video)
            video_file.length = video.file_url_length
            video_file.mimetype = video.file_url_mimetype
            video_file.save()
    ...
    def backwards(self, orm):
        ...
        Video = orm['localtv.video']
        VideoFile = orm['localtv.videofile']
        for video in Video.objects.annotate(file_count=models.Count('videofile')
                                            ).filter(file_count__gt=0):
            video_file = video.videofile_set.order_by('-length')[0]
            video.file_url = video_file.url
            video.file_url_length = video_file.length
            video.file_url_mimetype = video_file.mimetype
            video.save()
