...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        sitelocations = orm['localtv.SiteLocation'].objects.filter(site__id=settings.SITE_ID)
        if not sitelocations:
            import logging
            logging.error("Um, yikes, no SiteLocation.")
            return
        sitelocation = sitelocations[0]

        # Get or create a TierInfo
        tiers_infos = orm['localtv.TierInfo'].objects.filter(sitelocation=sitelocation)
        if not tiers_infos:
            orm['localtv.TierInfo'].objects.create(sitelocation=sitelocation)
            tiers_infos = orm['localtv.TierInfo'].objects.filter(sitelocation=sitelocation)
        tiers_info = tiers_infos[0]

        for field in ('payment_due_date', 'free_trial_available', 'in_free_trial', 'payment_secret', 'current_paypal_profile_id', 'free_trial_warning_sent', 'already_sent_welcome_email', 'inactive_site_warning_sent'):
            value = getattr(sitelocation, field)
            tiers_info.field = value
        tiers_info.save()
